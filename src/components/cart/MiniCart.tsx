...
export function MiniCart() {
  ...
  return <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="icon" className="relative bg-orange-500 hover:bg-orange-400">
          <ShoppingCart className="h-5 w-5" />
          {cart.totalItems > 0 && <Badge className="absolute -top-2 -right-2 px-1.5 py-0.5 bg-agrimarket-orange text-white">
              {cart.totalItems}
            </Badge>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80" align="end">
        <div className="flex flex-col">
          ...
          {cart.items.length === 0 ? <div className="py-8 text-center text-gray-500">
              Votre panier est vide
            </div> : <>
              <ScrollArea className="h-[300px]">
                <div className="space-y-3 pr-3">
                  {cart.items.map(item => <div key={item.id} className="flex flex-col p-2 border rounded-md">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <div className="flex items-center gap-1">
                          <Button variant="outline" size="icon" className="h-6 w-6 rounded-full p-0" onClick={() => handleDecreaseQuantity(item.id, item.quantity)} disabled={item.quantity <= 1}>
                            <Minus className="h-3 w-3" />
                          </Button>
                          <span className="text-xs font-medium min-w-[1.5rem] text-center">
                            {item.quantity}
                          </span>
                          <Button variant="outline" size="icon" className="h-6 w-6 rounded-full p-0" onClick={() => handleIncreaseQuantity(item.id, item.quantity)}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2 mt-2">
                  <Button className="w-full bg-agrimarket-orange hover:bg-orange-600" onClick={handleCheckout}>
                    Passer la commande
                  </Button>
