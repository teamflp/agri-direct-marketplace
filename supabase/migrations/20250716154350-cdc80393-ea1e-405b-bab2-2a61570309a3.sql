...
-- Create farmer_subscriptions table instead of subscriptions
...
-- RLS Policies for farmer_subscriptions
CREATE POLICY "Users can manage their own subscriptions" ON public.farmer_subscriptions FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Farmers can view their subscriptions" ON public.farmer_subscriptions FOR SELECT USING (
  EXISTS (SELECT 1 FROM farmers WHERE id = farmer_subscriptions.farmer_id AND user_id = auth.uid())
);
...
CREATE TRIGGER update_farmer_subscriptions_updated_at BEFORE UPDATE ON public.farmer_subscriptions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
